...
// Sanitize Unicode characters that cause HTTP header encoding issues
function sanitizeText(text: string): string {
  return text
    ...
    .replace(/[🤖🚀🔧📁✅❌⚠️]/g, ''); // Remove emoji characters
}
        ...
        try {
          ...
          const updateFileResponse = await fetch(
            `https://api.github.com/repos/${owner}/${repo}/contents/${fileResult.filename}`,
            {
              ...
              body: JSON.stringify({
                message: sanitizeText(`Fix issues in ${fileResult.filename}\n\n${fileResult.errors.map(e => `- ${e.message}`).join('\n')}`),
...
---
Generated by Self-Healing Code AI | Review changes carefully before merging`);
      ...
      const prResponse = await fetch(`https://api.github.com/repos/${owner}/${repo}/pulls`, {
        ...
        body: JSON.stringify({
          title: sanitizeText(`Auto-fix: ${totalIssues} code quality improvements`),
          head: newBranchName,
          base: branchName,
          body: prBody,
        }),
